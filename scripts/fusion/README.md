...
| 文件 | 职责 |
|---|---|
| `mappings.py` | 核心映射字典（CMES 21 + CHFS 8 + CFPS 5+8 条，9 字段含 wave；clean_rule DSL）、数据源可信度、按 wave 读取字段、入模特征（27 项） |
...
| CFPS 2018 | cfps2018famecon_202512.dta | 14,215 | 8 条 |
| CFPS 2020 | cfps2020famecon_202306.dta | 11,620 | 8 条 |
| CFPS 2022 | cfps2022famecon_202410.dta | 10,726 | 8 条 |
...
- B 验证 Spearman=**-0.062**（p=1.3e-44, n=50,580；预测分为正向特征均值，与负面信号负相关 = 方向正确）
- 关键特征缺失率已改善：BASIC_019 贷款 5.5%、BASIC_008 营收 15.5%、_cfps_private_debt 45.8%
- 模型产物：data/models/scorecard_v*.pkl（未注册，需 --register 或生产注册）

## 已知限制与后续迭代
1. **AUC 偏乐观**：合成标签来自同批指标（自我验证），B 验证（|0.06-0.10|）才是与真实信号的参考强度
2. **跨源缺失仍高**：CMES 专属特征（BASIC_003/004/005/0111_*、1041_02）缺失 >90% 被自动剔除；建议单一融合模型（评分卡缺失单独成箱天然处理）或按数据源分客群建模
3. **B 验证为特征均值占位**：train_fused_model 用 `feats.mean(axis=1)` 近似预测分，正式评估应改用 scorecard.predict_proba
4. **映射字典待扩展**：当前 34 条核心映射；field_catalog 9,282 字段可人工扩展（mapping_candidates.csv 是候选）
